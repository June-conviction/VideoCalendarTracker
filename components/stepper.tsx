...
const Stepper = React.forwardRef<HTMLDivElement, StepperProps>(
  ({ defaultValue = 0, value, onValueChange, orientation = "horizontal", className, ...props }, ref) => {
    ...
    const setActiveStep = React.useCallback(
      (step: number) => {
        // Allow moving to any step for debugging purposes
        if (value === undefined) {
    ...
    const setCompletedStep = React.useCallback((step: number) => {
...
const StepperItem = React.forwardRef<HTMLDivElement, StepperItemProps>(
  ({ step, completed = false, disabled = false, loading = false, className, children, ...props }, ref) => {
    ...
    const isCompleted = completed || completedSteps.has(step)
    const state: StepState =
      isCompleted || step < activeStep ? "completed" : activeStep === step ? "active" : "inactive"

    const isLoading = loading && step === activeStep
    const isDisabled = disabled || (step > activeStep && !completedSteps.has(step - 1))
